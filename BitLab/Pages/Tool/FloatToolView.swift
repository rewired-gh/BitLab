...
class FloatToolViewModel: ObservableObject {
  ...
  var exponentShiftedDecString: String {
    ...
    let exponent = Int(numberExponent!)
    switch precision {
    case .single:
      if exponent == 0 {
        return String(-126)
      }
      return String(exponent - 127)
    case .double:
      if exponent == 0 {
        return String(-1022)
      }
      return String(exponent - 1023)
    }
  }
  ...
  var fractionBinString: String {
    guard numberFraction != nil && numberExponent != nil else {
      return "Unavailable"
    }
    let exponent = Int(numberExponent!)
    ...
    switch precision {
    case .single:
      return (exponent == 0 ? "0." : "1.") + str.leftPad(with: "0", toLength: 23)
    case .double:
      return (exponent == 0 ? "0." : "1.") + str.leftPad(with: "0", toLength: 52)
    }
  }
}
